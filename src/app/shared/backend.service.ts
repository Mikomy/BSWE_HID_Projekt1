...
export class BackendService {
  ...
  public addChildData(child: Child, page: number): Observable<any> {
    this.storeService.setLoading(true);// Set loading to true before adding child
    return this.http.post('http://localhost:5000/childs', child).pipe(
      map(_ => {
        this.getChildren(page).subscribe(() => {
          this.storeService.setLoading(false);// Set loading to false after adding child
        });
      })
    );
  }
  ...
  public deleteChildData(childId: string, page: number): Observable<void> {
    this.storeService.setLoading(true); // Set loading to true before deleting child
    return this.http.delete(`http://localhost:5000/childs/${childId}`, { observe: 'response' })
      .pipe(
        map(response => {
          this.storeService.setLoading(false); // Set loading to false after deleting child
        })
      );
  }
